...
import com.livestormer.accounts.owners.OwnerController;
...
privileged aspect OwnerController_Roo_Controller_Json {
    ...
    @ResponseBody
    public ResponseEntity<String> OwnerController.jsonFindOwnersByUsernameEquals(@RequestParam("username") String username) {
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Type", "application/json; charset=utf-8");
        return new ResponseEntity<String>(Owner.toJsonArray(Owner.findOwnersByUsernameEquals(username).getResultList()), headers, HttpStatus.OK);
    }
