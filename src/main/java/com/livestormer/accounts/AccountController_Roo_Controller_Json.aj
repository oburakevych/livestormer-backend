...
import com.livestormer.accounts.AccountController;
...
privileged aspect AccountController_Roo_Controller_Json {
    ...
    @ResponseBody
    public ResponseEntity<String> AccountController.jsonFindAccountsByOwner(@RequestParam("owner") Owner owner) {
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Type", "application/json; charset=utf-8");
        return new ResponseEntity<String>(Account.toJsonArray(Account.findAccountsByOwner(owner).getResultList()), headers, HttpStatus.OK);
    }
